...
But this exception is limited to the repeating the exact same test. This
warns:
...
tool, does not know if `guarded()` is deterministic or talks to a
database. For variables (e.g. `guarded` versus `guarded()`), this is
less of an issue, so in this case, `possibly-used-before-assignment`
acts more like a future-proofing style preference than an error, per se.
